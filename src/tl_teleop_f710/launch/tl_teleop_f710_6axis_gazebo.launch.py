"""天链机械臂 6 轴 + F710 手柄遥操作 — 通用 Gazebo 仿真启动文件。

IK 由仿真桥接节点内部使用 Pinocchio 本地求解，无需 MoveIt2。

用法：
  ros2 launch tl_teleop_f710 tl_teleop_f710_6axis_gazebo.launch.py arm_type:=tcb605
  ros2 launch tl_teleop_f710 tl_teleop_f710_6axis_gazebo.launch.py arm_type:=tcb605 joy_dev:=/dev/input/js1
"""
...
def generate_launch_description():
    ...
    config_path = os.path.join(tl_teleop_f710_share, "config", "tl_teleop_f710_6axis_sim.yaml")
    ...
    gazebo_launch_path = PathJoinSubstitution(
        [
            tl_gazebo_share,
            "launch",
            "gazebo_6axis_f710_sim.launch.py",
        ]
    )
    ...
    return LaunchDescription(
        [
            DeclareLaunchArgument(
                ...
                description="机械臂型号，如 tcb605、tcb610、tcb705",
            ),
            DeclareLaunchArgument(
                "joy_dev",
