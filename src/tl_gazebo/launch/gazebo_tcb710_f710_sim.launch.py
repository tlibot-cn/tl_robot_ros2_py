"""天链机械臂 F710 遥操作仿真专用启动文件（Gazebo + position controller 模式）。

与 gazebo_tcb710_demo.launch.py 的区别：
  - 使用 ros2_controllers_f710_sim.yaml（仅加载 position controller，无 trajectory controller）
  - 不加载 tcb_group_controller，避免接口冲突
  - 用于 F710 手柄遥操作仿真测试
"""
...
def generate_launch_description():
    ...
    urdf_model_path = os.path.join(pkg_share, 'config/gazebo_tcb710_f710_sim.urdf.xacro')
