...
天链（TianLian）机械臂 ROS2 工作空间（Python 实现，含一个 C++ 硬件接口插件）。`src/` 下包含 10 个功能包（其中 `tl_moveit2_config` 内含 14 个子功能包），使用标准 `colcon build` 构建流程。纯 ROS2（ament_cmake + ament_python），无 Node.js、无前端。
